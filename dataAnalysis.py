...
def calculate_iat_d6_score_from_custom_blocks(
    ...
    for block_label, block_df_orig in block_dfs_raw.items():
        ...
        if len(block_df_filtered) < min_valid_trials_for_block_mean:
            print(f"Warning ({log_pid_label}): < {min_valid_trials_for_block_mean} valid trials in {block_label}. skip.") 
            return np.nan
...
if not list_of_dataframes:
    ...
    print(f"Using Condition1 for B3/B4 (e.g., Incongruent): '{CONDITION1_FOR_BLOCK_3_4}' ({N_TRIALS_B3} pract, {N_TRIALS_B4} test)")
    print(f"Using Condition1 for B6/B7 (e.g., Congruent): '{CONDITION1_FOR_BLOCK_6_7}' ({N_TRIALS_B6} pract, {N_TRIALS_B7} test)")
    ...
    if all_d_scores_calculated:
        ...
        if len(d_scores_array) >= 2:
            ...
            scores_df = pd.DataFrame({'id': [f"P{pid}" for pid in participant_ids_for_scores], 'd_score': d_scores_array})
